...
- I asked ChatGPT to help me with how to cut the Last.CASI.Score into quantile-based groups in Question 3G.
...
involve coding (in any language) so far and how familiar/comfortable you are in coding in R. (Note: We do not expect you to have 
taken any such courses already.)
- BIOST514, BIOST515, BIOST546
...
- To gain more familiarity with RStudio, git and Github; to learn how to write R packages; to get more comfortable navigating through terminal; to learn computing skills in general, which might be useful in biostats research.
...
<u>**Answer**</u>: 
-The 'generate_data' function takes in a positive integer input 'n' and generates a numeric vector of length n, where each element is assigned one of three distributions based on a randomly chosen index: a normal distribution with a mean of 10 and standard deviation of 1, a gamma distribution with shape and scale parameters both set to 2, or a chi-squared distribution with 3 degrees of freedom. The choice of distribution for each position is determined by an index vector (idx_vec) with each element randomly assigned a value of 1, 2, or 3. Depending on the value in the index vector, each position in the output vector (sample_vec) is assigned with one of the three distributions.\
...
<u>**Answer**</u>: 
```{r clt, out.width = "600px", fig.align="center", fig.cap=c("CLT PLOT")}
# Function to perform trials and calculate means
repeat_mean_calculation <- function(n, trials = 10000) {
  means <- replicate(trials, mean(generate_data(n)))
  return(means)
}
...
n_values <- c(1, 2, 3, 5, 10, 100)

# Perform trials and plot histograms
for (n in n_values) {
  means <- repeat_mean_calculation(n)
  hist(means, breaks = 100, main = paste("n =", n), xlab = "Empirical Means", ylab = "Frequency")
}
```
...
<u>**Answer**</u>: 
The plot verifies the Central Limit Theorem by showing that as the n increases, the distribution of empirical means (computed from randomly generated data with varying distributions) becomes more normal (and it doesn't matter what the original distribution of the data is). This is evidenced by the histograms becoming more symmetric and bell-shaped, particularly noticeable as n moves from 1 to 100. As the sample size converges to infinity (sufficiently large, 100 in this plot) the means approximate a normal distribution, aligning with what's stated by clt.\
...
<u>**Answer**</u>: The head(df) command displays the first six rows of the dataframe df and summary(df) provides a statistical summary of each column (variables) in the dataframe. 
...
print(class(df))
```
df has 84 rows and 7 columns, indicating that there are 84 individual donors. df is a dataframe.\
...
hist(df$Age.at.Death, main = "Histogram of Age at Death", xlab = "Age at Death", ylab = "Frequency", col = "salmon")
...
summary(df)
```
The summary here (in Question 3E) is more informative because it provides a breakdown of categorical data (e.g., counts of Sex, APOE4.Status, Cognitive.Status, and Braak levels), showing the distribution across different categories, in addition to the numerical summaries for variables like Age.at.Death and Last.CASI.Score.\
...
<u>**Answer**</u>:
```{r}
table1 <- table(df$Braak, df$Cognitive.Status)
print(table1)
```
...
<u>**Answer**</u>:
```{r}
quantiles <- quantile(df$Last.CASI.Score, probs = seq(0, 1, by = 0.25), na.rm = TRUE)
df$Quartiles <- cut(df$Last.CASI.Score, breaks = quantiles, include.lowest = TRUE)
casi_cognitive_table <- table(df$Quartiles, df$Cognitive.Status)
print(casi_cognitive_table)
```
Higher CASI scores, which indicate better cognitive function, are associated with a lower incidence of dementia. Conversely, lower CASI scores are more frequently associated with dementia, underlining the test's potential utility in assessing the risk or presence of cognitive impairment.\
