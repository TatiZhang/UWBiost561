...
- I asked ChatGPT how to align the colors of the regression lines with the colors of points of the 3 different species in Q2(B).\
- I also asked it what 'clique_fraction' and 'density_low'are in Q3.\
.
...
knitr::include_graphics("https://raw.githubusercontent.com/linnykos/561_s2024_public/main/HW2_files/population.png")
```
<u>**Answer**</u>: 
```{r}
country_population_filtered <- country_population %>%
  filter(year >= 1990 & year <= 2010)
mean_population_by_country <-country_population_filtered %>%
  group_by(iso3c,country) %>%
  summarize(mean_population=round(mean(population, na.rm = TRUE),0),.groups = 'drop')
mean_population_by_country
```
...
knitr::include_graphics("https://raw.githubusercontent.com/linnykos/561_s2024_public/main/HW2_files/birth.png")
```

<u>**Answer**</u>: 
```{r}
country_birth_expect_filtered <- country_birth %>%
  filter(year >= 1990 & year <= 2010) %>% 
  select(iso3c, year, life_expect) %>%              
  na.omit() %>%             # Remove rows with any NA values
  group_by(iso3c) %>%                                
  summarize(
    max_life_expect = round(max(life_expect, na.rm = TRUE),1),
    min_life_expect = round(min(life_expect, na.rm = TRUE),1)
  ) %>%
  mutate(range_life = max_life_expect - min_life_expect)
country_birth_expect_filtered
...
```{r}
dim_country_both <- dim(country_both)
dim_country_both

```
There are 343980 rows and 8 columns in  `country_both`.\
...
knitr::include_graphics("https://raw.githubusercontent.com/linnykos/561_s2024_public/main/HW2_files/scatterplot-colored.png")
```
<u>**Answer**</u>:
```{r}
penguins %>%
  ...
  # + xlim(0,20) + ylim(0,7) # adjust axis
```
...
knitr::include_graphics("https://raw.githubusercontent.com/linnykos/561_s2024_public/main/HW2_files/facet.png")
```
...
penguins %>%
  ...
  scale_color_manual(values = c("Adelie" = "cornflowerblue",
                                "Chinstrap" = "coral2",
                                "Gentoo" = "goldenrod3"))+
  # Add linear regression lines without confidence bands
...
<u>**Answer**</u>:
The `generate_random_graph()` function creates a random adjacency matrix (adj_mat) that represents a graph with a specified number of nodes (n). 'adj_mat' is a square matrix where both rows and columns represent nodes in a graph, and the entries (0 or 1) indicate the absence or presence of an edge between nodes. 'clique_fraction' determines the proportion of these nodes that will form a clique (a subset of nodes that are all connected to each other). 'density_low' parameter sets the density of edges outside the clique, i.e., the probability of an edge existing between any two nodes not in the clique.\
...
<u>**Answer**</u>:
The 'pivot_longer_heatmap()' function turns 'adj_mat' from its standard wide format into a long format. It converts the 'adj_mat' so that each row of the new data frame represents a single element of the matrix. 
...
knitr::include_graphics("https://raw.githubusercontent.com/linnykos/561_s2024_public/main/HW2_files/adj_mat.png")
```
<u>**Answer**</u>:
```{r}
# Create the heatmap
heatmap1 <- ggplot(mat_long, aes(x = X, y = Y, fill = as.factor(Value))) +
  geom_tile() +  # Use geom_tile to create the heatmap
  scale_fill_manual(values = c("0" = "palegoldenrod", "1" = "coral2")) +  # Mapping colors
  coord_fixed(ratio = 1) +  #aspect ratio is 1:1
  labs(x = "Column", y = "Row", fill = "Value") +  
  theme_minimal() +  # Use a minimal theme
  theme(axis.text = element_text(size = 12),  # Adjust text size for clarity
        legend.title = element_text(size = 14))  # Adjust legend title size
print(heatmap1)
```
The resulting heatmap is a visual representation of 'adj_mat', where the layout of tiles mirrors the matrix structure. Each tile's position corresponds to an element in adj_mat, and the color indicates whether an edge exists between the nodes. Palegoldenrod means no edge exists and coral means that exists an edge between these two nodes. \
...
knitr::include_graphics("https://raw.githubusercontent.com/linnykos/561_s2024_public/main/HW2_files/adj_mat2.png")
```
<u>**Answer**</u>:
```{r}
# Convert adj_mat2 to long format using provided function
mat_long2 <- pivot_longer_heatmap(adj_mat2)
heatmap1 <- ggplot(mat_long2, aes(x = X, y = Y, fill = as.factor(Value))) +
  geom_tile() +  # Use geom_tile to create the heatmap
  scale_fill_manual(values = c("0" = "palegoldenrod", "1" = "coral2")) +  # Mapping colors
  coord_fixed(ratio = 1) +  #aspect ratio is 1:1
  labs(x = "Column", y = "Row", fill = "Value") +  
...
<u>**Answer**</u>:
- Test on data with known outcomes by comparing the function's output with expected results in these controlled scenarios.\
- Test the function by changing parameters like the number of nodes, density of edges and other variety of conditions, or randomly generating inputs.\
- Test the edges/bounds of input values that the function can handle. For example, what happens if the function receives the smallest or largest possible graph, or graphs with unusual configurations.\
- Ask a few different people to run it on the same data to see if they get the same results. Or run on different several machines.\
